...
if 'optimized_route' not in st.session_state: st.session_state.optimized_route = None
if 'total_km' not in st.session_state: st.session_state.total_km = 0
...
def calculate_tsp(start_coords, destinations_df):
    ...
    if unvisited.empty: return None, 0
...
if st.session_state.user_plate is None:
    ...
    t1, t2, t3 = st.tabs(["🌎 Map Overview", "🗺️ Smart Routing", "📦 Unloading"])
    ...
    with t1:
        ...
        st_folium(m1, width="100%", height=450, key="overview_map")

    with t2:
        st.subheader("Optimal Sequential Path")
        if st.button("⚙️ Generate Optimal Route", type="primary", width="stretch"):
            route_data, km = calculate_tsp(curr_loc, user_data.drop_duplicates(subset=['City_Clean']))
            st.session_state.optimized_route = route_data
            st.session_state.total_km = km
            st.rerun()
        ...
        if st.session_state.optimized_route:
            ...
            for i, r in enumerate(st.session_state.optimized_route):
            ...
            st.success(f"Trip: {st.session_state.total_km:.1f} km")
            st_folium(m2, width="100%", height=500, key="persistent_route_map")
    ...
    with t3:
        ...
        col1.metric("Load", f"{tot_kg:,.1f} KG")
        ...
        if st.button("⏹️ Complete & Sync", type="primary", width="stretch"):
            if st.session_state.start_time:
                ...
                st.error("Error: Start timer first.")
